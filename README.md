...
## 🌐 GUI
С последним обновлением TaskManagerAPI получил полноценный веб-интерфейс для работы с задачами прямо через браузер:
- Просмотр всех задач с динамическим списком
- Создание, редактирование, завершение и удаление задач через UI
- Автоматическое обновление списка при изменении
- Стильная, адаптивная тёмная тема

Запуск веб-интерфейса:
1. Сервер запускается как обычно:
```bash
go run cmd/server/main.go
```

2. После старта автоматически открывается браузер с интерфейсом по адресу:
```bash
http://localhost:8080/
```
